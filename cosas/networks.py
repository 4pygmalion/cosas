...
class PyramidSeg(torch.nn.Module):
    def __init__(self):
        ...
        self.level0.encoder._conv_stem = Conv2dStaticSamePadding(
            3, 48, kernel_size=(3, 3), stride=(2, 2), bias=False, image_size=224
        )
        self.level1 = smp.FPN(
            encoder_name="efficientnet-b4",
            encoder_weights="imagenet",
        ...
        self.level1.encoder._conv_stem = Conv2dStaticSamePadding(
            5, 48, kernel_size=(3, 3), stride=(2, 2), bias=False, image_size=224
        )
        self.level2 = smp.FPN(
            encoder_name="efficientnet-b4",
            encoder_weights="imagenet",
    ...
    def forward(self, xs: torch.Tensor):
        ...
        for x in xs:
            ...
            downsampled_o = torch.nn.functional.interpolate(
                level0_fusion, self.level1_size
            )
            level1_inputs = tesellation(downsampled_o)  # (1, N, 5, 224*3, 224*3)
            ...
            downsampled_x = Resize(self.level1_size)(x)
            level1_fusion = torch.concat(
                [downsampled_x, level1_output], axis=0
            ).unsqueeze(0)
