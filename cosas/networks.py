...
class MultiTaskAE(torch.nn.Module):
    def __init__(self, architecture: str, encoder_name, input_size=(224, 224)):
        ...
        self.stain_vec_decoder = self.architecture.decoder
        self.stain_vec_head = self.architecture.segmentation_head

        self.stain_den_decoder = UnetDecoder(
            encoder_channels=self.encoder.out_channels,
            decoder_channels=(256, 128, 64, 32, 16),
        )
        self.stain_den_head = SegmentationHead(
            in_channels=16, out_channels=2, activation=None
        )
...
class Siamformer(torch.nn.Module):
    ...
    def _forward_high_mag_one_image(self, x: torch.Tensor):
        ...
        segmenter_output = self.model.forward(x)  # (N, c, H/4, W/4)
        logits = segmenter_output.logits

        return torch.nn.functional.interpolate(logits, size=self.size, mode="bilinear")

    def forward(self, x: torch.Tensor):
        original_size = x.shape[-2:]

        logit_low_mag = self._forward_low_mag(x)
        logit_igh_mag = self._forward_high_mag_one_image(x)
        z = torch.concat([logit_low_mag, logit_igh_mag], dim=1)  # (N, 2, W, H)
